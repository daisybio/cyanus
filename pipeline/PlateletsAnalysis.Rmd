...
path_to_rds <- "../data/platelets/sceClusteredFiltered.rds"
sce <- readRDS(path_to_rds)
```
...
# You can also select samples that should not be included
...
```{r}
sce <- makePatientSelection(sce = sce, deselected_patients = c("RPS 108", "RPS 096", "RPS 149"))
#sce <- makeSampleSelection(sce, c("149A_dual"))
sce <- clusterSCE(sce)
...
CATALYST::plotAbundances(sce, "meta10", by = "cluster_id", group_by = "patient_id", shape_by = "dual_triple")

invisible(plotStarsCustom(sce, backgroundValues = cluster_codes(sce)[["meta10"]]))
```
...
```{r}
parameters <- prepDiffExp(sce, contrastVars = c("dual_triple", "activated_basline"), colsDesign = c("dual_triple", "activated_baseline"), method = "diffcyt-DA-edgeR")
out_edgeR <- diffcyt::diffcyt(d_input = sce,
...
CATALYST::plotPbExprs(sce, k = "meta10", features = "state", color_by = "dual_triple", 
              facet_by = "cluster_id", shape_by = "dual_triple")

CATALYST::plotPbExprs(sce, features = "state", facet_by = "antigen", color_by = "dual_triple", shape_by = "dual_triple")
CATALYST::plotDiffHeatmap(sce, rowData(out$res), all = T)
```


## Comparison of stimulated and unstimulated platelets of dual patients
First, make the subselection of dual patients.
```{r}
...
out_limma <- diffcyt::diffcyt(d_input = sce_dual_ab,
                 design = parameters[["design"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
...
diffcyt::topTable(out_limma$res, order_by="p_adj", all=TRUE)
```

And then visualize both:
```{r, fig.width=6, fig.height=4}
CATALYST::plotPbExprs(sce_dual_ab, k = "all", features = "state", color_by = "activated_baseline")
CATALYST::plotPbExprs(sce_dual_ab, features = "state", facet_by = "antigen", color_by = "activated_baseline")
CATALYST::plotDiffHeatmap(sce_dual_ab, rowData(out_limma$res), all = T)
```


## Comparison of stimulated and unstimulated platelets of triple patients
First, make the subselection of dual patients and cluster again.
```{r}
allsamples <- unique(colData(sce)$sample_id)
triple_ab <- grep("triple",allsamples,value=TRUE)
sce_triple_ab <- filterSCE(sce, sample_id %in% triple_ab) # not using makeSampleSelection triple_ab are the including samples
```
...
out_limma <- diffcyt::diffcyt(d_input = sce_triple_ab,
                 design = parameters[["design"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-limma",
...
top_limma <- diffcyt::topTable(out_limma$res, order_by="p_adj", all=TRUE)
top_limma
```

And then visualize both:
```{r, fig.width=6, fig.height=4}
CATALYST::plotPbExprs(sce_triple_ab, k = "all", features = "state", color_by = "activated_baseline")
CATALYST::plotPbExprs(sce_triple_ab, features = "state", facet_by = "antigen", color_by = "activated_baseline")
CATALYST::plotDiffHeatmap(sce_triple_ab, rowData(out_limma$res), all = T)
```
:plotDiffHeatmap(sce, rowData(out$res), all = T)
```
