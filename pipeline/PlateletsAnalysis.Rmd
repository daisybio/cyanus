...
CATALYST::plotPbExprs(sce, features = "state", facet_by = "antigen", color_by = "dual_triple", shape_by = "activated_baseline")
...
param <- MulticoreParam(workers = 22, progressbar = TRUE)
register(param)

system.time(emd_result <- SigEMD(sce, "all", "dual_triple", nperm=100, parallel=TRUE))
# user   system  elapsed 
# 3512.914  434.689 3949.872 

print(emd_result$all$emdall)
...
test on pbmc data
```{r, eval=FALSE}
scePBMC <- readRDS("~/hiwi/cytof/data/pbmc/sceTransformed.rds")
scePBMC <- cluster(scePBMC)
scePBMC <- mergeClusters(scePBMC, k = "meta20", id = "all", table = data.frame(old_cluster = seq_len(20), new_cluster = "all"))
...
system.time(emd_result <- SigEMD(scePBMC, "all", "condition", nperm=10, parallel=TRUE))
# user   system  elapsed 
# 3512.914  434.689 3949.872 
