...
CATALYST::plotAbundances(sce, "meta9", by = "cluster_id", shape_by = "patient_id", group_by = "activated_baseline")
```

```{r, fig.width=6, fig.height=4}
invisible(plotStarsCustom(sce, backgroundValues = cluster_codes(sce)[["meta9"]]))
```

```{r}
CATALYST::plotClusterExprs(sce, "meta9")

CATALYST::plotFreqHeatmap(sce, "meta9")
```
...
for( method in DR_methods ){
  print(method)
  sce <- runDimRed(sce, method, cells_chosen = 300, feature_chosen = "state", assay_chosen = "exprs", scale = T, k = 3)
}
```
...
for( method in DR_methods ){
  print(method)
  sce <- runDimRed(sce, method, cells_chosen = 300, feature_chosen = "type", assay_chosen = "exprs", scale = T, k = 3)
}
```
...
for( method in DR_methods ){
  print(method)
  sce <- runDimRed(sce, method, cells_chosen = 300, feature_chosen = NULL, assay_chosen = "exprs", scale = T, k = 3)
}
```
...
CATALYST::plotPbExprs(sce, k = "all", features = NULL , color_by = "activated_baseline", 
              facet_by = "antigen", shape_by = "dual_triple")

CATALYST::plotPbExprs(sce, k = "meta6", features = NULL , color_by = "activated_baseline", 
              facet_by = "cluster_id", shape_by = "dual_triple")

```

The plot show the median marker expression of state and type markers colored by activated and baseline conditions. The state marker CD62P shows the biggest difference in activated and baseline platelets. The same applies for marker CD63. This state marker is also more expressed in activated platelets than in baseline platelets. 
...
CATALYST::plotPbExprs(sce, k = "all", features = NULL, color_by = "dual_triple", 
              facet_by = "antigen", shape_by = "activated_baseline")

CATALYST::plotPbExprs(sce, k = "meta6", features = NULL, color_by = "dual_triple", 
...
```{r,fig.with=12, fig.height=8}
CATALYST::plotDiffHeatmap(sce_d, rowData(res$limma$res), all = T)
```
