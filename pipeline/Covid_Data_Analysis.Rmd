...
md <- 
  read_excel(
    "/nfs/home/students/l.arend/data/covid/meta_11vs8_batch.xlsx"
  )
...
saveRDS(sce_covid, "/nfs/home/students/l.arend/data/covid/sce_transformed.rds")

sce_covid <- readRDS("/nfs/home/students/l.arend/data/covid/sce_transformed.rds")
sce_covid <- clusterSCE(sce_covid)


plotPbExprs(sce_covid, color_by = "platelets", features = NULL)
...
```{r}
sceBaseline <- filterSCE(sce_covid, platelets == "B")

CATALYST::pbMDS(sceBaseline,
                label_by = "sample_id",
                color_by = "covid",
                features = NULL,
                assay = "exprs")
...
```{r}
CATALYST::plotExprs(sceBaseline, color_by = "covid")
```

```{r}
CATALYST::plotExprs(sceBaseline, color_by = "patient_id")
```



```{r}
CATALYST::plotExprHeatmap(sceBaseline, scale="never")
```
...
# diffcyt methods
res_baseline <- runDS(sce = sce_baseline,
      condition = "covid",
...
# diffcyt methods
res_activated <- runDS(sce = sce_activated,
      condition = "covid",
      de_methods = c("LMM", "limma"),
      k = "all",
      features = "all"
