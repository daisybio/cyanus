...
```{r}
createVennDiagram(res)
```

Show heatmaps of the different methods:
```{r}
plotDiffHeatmap(tmp, rowData(res$limma$res), all=TRUE, k="all")

plotDiffHeatmap(tmp, rowData(res$LMM$res), all=TRUE, k="all")

plotDiffHeatmap(tmp, res$SigEMD$overall, all=TRUE, k="all")

plotDiffHeatmap(tmp, res$DEsingle, all=TRUE, k="all")

```

Let's add the none markers as state markers to check their influence.
```{r}
rowData(tmp)$marker_class <- rowData(tmp)$marker_class[rowData(tmp)$marker_class == "none"] <- "state"

res_none <- runDS(sce = tmp,
      condition = "activated_baseline",
      de_methods = c("limma","LMM", "SigEMD", "DEsingle"),
      k = "all",
      features = "all",
...
ei <- metadata(tmp)$experiment_info

# sample A and B 
set.seed(1234)
condition <- as.character(ei$activated_baseline[sample(nrow(ei), nrow(ei), replace=FALSE)])

# edit activated_baseline condition in experiment_info
ei$activated_baseline <- condition
metadata(tmp)$experiment_info <- ei

# edit activated_baseline in colData (not corrected in sample name)
coldata_condition <- rep(condition, each=10000)
colData(tmp)$activated_baseline <- as.factor(coldata_condition)

res_sampled <- runDS(sce = tmp,
      condition = "activated_baseline",
      de_methods = c("limma","LMM", "SigEMD", "DEsingle"),
      k = "all",
      features = "all",
...
lapply(res_sampled$SigEMD$all$nonHur_gene, function(marker) plot_emd_density_sig(res_sampled$SigEMD$all, "PAR1"))
```
