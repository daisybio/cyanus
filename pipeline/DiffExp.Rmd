...
# Ab hier is schon alles im runDS
...
plotDiffHeatmap(sce_dual_ab, rowData(limma_dual_ab$res), all = T)
...
data.table::as.data.table(diffcyt::topTable(limma_dual_ab$res, all=TRUE))
data.table::as.data.table(diffcyt::topTable(LMM_dual_ab$res, all=TRUE))

CATALYST::plotDiffHeatmap(x = sce_dual_ab,
                          y = rowData(limma_dual_ab$res), 
                          all = T)
CATALYST::plotDiffHeatmap(x = sce_dual_ab,
...
First, make the subselection of triple patients to compare activated and baseline platelets.
```{r, dualAB_selection}
allsamples <- unique(colData(sce)$sample_id)
triple_ab <- grep("triple",allsamples,value=TRUE)
sce_triple_ab <- filterSCE(sce, sample_id %in% triple_ab)
```

Running diffcyt methods:
```{r, diffcyt}
parameters <- prepDiffExp(sce_triple_ab, contrastVars = c("activated_baseline"), colsDesign = c("activated_baseline"), method = "diffcyt-DS-limma")

#blockID <- metadata(sce_triple_ab)$experiment_info[["patient_id"]]

limma_triple_ab <- diffcyt::diffcyt(d_input = sce_triple_ab,
                 design = parameters[["design"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-limma",
...
parameters <-  prepDiffExp(sce_triple_ab, contrastVars = c("activated_baseline"), colsFixed = c("activated_baseline"), colsRandom=c("patient_id"), method = "diffcyt-DS-LMM")


LMM_triple_ab <- diffcyt::diffcyt(d_input = sce_triple_ab,
                 formula = parameters[["formula"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-LMM",
...
data.table::as.data.table(diffcyt::topTable(limma_triple_ab$res, all=TRUE))
data.table::as.data.table(diffcyt::topTable(LMM_triple_ab$res, all=TRUE))

CATALYST::plotDiffHeatmap(x = sce_triple_ab,
                          y = rowData(limma_triple_ab$res), 
                          all = T)
CATALYST::plotDiffHeatmap(x = sce_triple_ab,
                          y = rowData(LMM_triple_ab$res), 
                          all = T)
```

## Dual Activated vs. Triple Activated
First, make the subselection of activated platelets of dual and triple patients
```{r, dualAB_selection}
allsamples <- unique(colData(sce)$sample_id)
a_dual_triple <- grep("A",allsamples,value=TRUE)
sce_a_dual_triple <- filterSCE(sce, sample_id %in% a_dual_triple)
```

Running diffcyt methods:
```{r, diffcyt}
parameters <- prepDiffExp(sce_a_dual_triple, contrastVars = c("dual_triple"), colsDesign = c("dual_triple"), method = "diffcyt-DS-limma")

limma_a_dual_triple <- diffcyt::diffcyt(d_input = sce_a_dual_triple,
                 design = parameters[["design"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-limma",
...
parameters <-  prepDiffExp(sce_a_dual_triple, contrastVars = c("dual_triple"), colsFixed = c("dual_triple"), colsRandom=c("patient_id"), method = "diffcyt-DS-LMM")

LMM_a_dual_triple <- diffcyt::diffcyt(d_input = sce_a_dual_triple,
                 formula = parameters[["formula"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-LMM",
...
data.table::as.data.table(diffcyt::topTable(limma_a_dual_triple$res, all=TRUE))
data.table::as.data.table(diffcyt::topTable(LMM_a_dual_triple$res, all=TRUE))

CATALYST::plotDiffHeatmap(x = sce_a_dual_triple,
                          y = rowData(limma_a_dual_triple$res), 
                          all = T)
CATALYST::plotDiffHeatmap(x = sce_a_dual_triple,
                          y = rowData(LMM_a_dual_triple$res), 
                          all = T)
```
...
```{r, dualAB_selection}
allsamples <- unique(colData(sce)$sample_id)
b_dual_triple <- grep("B",allsamples,value=TRUE)
sce_b_dual_triple <- filterSCE(sce, sample_id %in% b_dual_triple)
```

Running diffcyt methods:
```{r, diffcyt}
parameters <- prepDiffExp(sce_b_dual_triple, contrastVars = c("dual_triple"), colsDesign = c("dual_triple"), method = "diffcyt-DS-limma")

limma_b_dual_triple <- diffcyt::diffcyt(d_input = sce_b_dual_triple,
                 design = parameters[["design"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-limma",
...
parameters <-  prepDiffExp(sce_b_dual_triple, contrastVars = c("dual_triple"), colsFixed = c("dual_triple"), colsRandom=c("patient_id"), method = "diffcyt-DS-LMM")

LMM_b_dual_triple <- diffcyt::diffcyt(d_input = sce_b_dual_triple,
                 formula = parameters[["formula"]],
                 contrast = parameters[["contrast"]],
                 analysis_type = "DS",
                 method_DS = "diffcyt-DS-LMM",
...
data.table::as.data.table(diffcyt::topTable(limma_b_dual_triple$res, all=TRUE))
data.table::as.data.table(diffcyt::topTable(LMM_b_dual_triple$res, all=TRUE))

CATALYST::plotDiffHeatmap(x = sce_b_dual_triple,
                          y = rowData(limma_b_dual_triple$res), 
                          all = T)
CATALYST::plotDiffHeatmap(x = sce_b_dual_triple,
                          y = rowData(LMM_b_dual_triple$res), 
                          all = T)
```
