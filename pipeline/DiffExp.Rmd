...
```{r}
res_perm_samples <- SigEMD(sce_dual, "all", "activated_baseline", Hur_gene = rownames(sce_dual), nperm = 100, parallel = TRUE, permute_samples = TRUE)
...
```{r}
plotDiffHeatmap(sce_dual, rowData(res_dual$limma$res), all=TRUE, k="all")

plotDiffHeatmap(sce_dual, rowData(res_dual$LMM$res), all=TRUE, k="all")

plotDiffHeatmap(sce_dual, res_dual$SigEMD$overall, all=TRUE, k="all")

plotDiffHeatmap(sce_dual, res_dual$DEsingle, all=TRUE, k="all")
...
```{r, eval = FALSE}
timed1 <- system.time(emd_result <- SigEMD(sce_dual, "all", "activated_baseline", nperm=100, parallel=TRUE, permute_samples = TRUE))
#     user   system  elapsed 
...
lapply(state_markers(sce_dual), function(marker) plot_emd_density_sig(emd_result_allHur, marker))
```


```{r, eval = FALSE}
# now check if what happens if we skip the logistic regression
timed <- system.time(emd_result_allHur <- SigEMD(sce_dual, "all", "activated_baseline", Hur_gene=rownames(sce_dual), nperm=100, parallel=TRUE, permute_samples = TRUE))
print(timed)
...
print(emd_result$emdall[marker_id %in% emd_result$nonHur_gene, ])
print(emd_result_allHur$emdall[marker_id %in% emd_result$nonHur_gene, ])
```
