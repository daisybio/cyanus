...
calculate_single<- function(data=data, condition=condition,Hur_gene=Hur_gene, binSize, nperm, parallel=FALSE){
 ...
 require(arm)
  ...
  if(length(Hur_gene)!=0){
    ...
    Hur.result <- calculate_emd_hur(data[Hur_gene, , drop=F], condition, binSize, nperm, parallel=parallel)
  }
  
  if(length(nonHur_gene)!=0){
    message("do EMD for nonHur genes.")
    nonHur.result <- calculate_nonzero(data[nonHur_gene,, drop=F], databinary[nonHur_gene,, drop=F], condition, nperm, binSize)
  }
...
calculate_emd_gene_sig <- function(datalist, binSize) {
...
.emd_pairwise_qval_sig <- function(emd, emd.perm,nperm ) { 
   ...
   pvals<- apply(temp,1, function(x){(sum(x[-1]>=as.numeric(x[1])) + 1)/(nperm + 1)})
