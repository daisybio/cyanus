...
calculate_emd_hur <- function(data, outcomes, binSize,
  ...
  for (i in 1:dim(data)[1]){
    qvals[i,]<- (sum(abs(emd.perm[i,])>= abs(as.numeric(emd[i,1]))) + 1)/(dim(emd.perm)[2] + 1)
...
calculate_emd_gene <- function(vec, outcomes, sample_names, binSize) {
...
.emd_gene_pairwise <- function(vec, idxA, idxB, binSize) {
  ...
  emdist::emd2d(densA, densB, max.iter=1000)
}
