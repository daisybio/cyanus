...
ggplot(times, aes(x = reorder(method,elapsed), y = elapsed, fill = method)) +
  geom_bar(size = 3, stat = "identity") +
  theme_bw() + theme(text = element_text(size = 18), axis.text.x = element_text(angle = 45, hjust=1)) + xlab(label="method") + ggtitle("Dual Platelets with Random Effect")
...
ggplot(tmp, aes(marker_id, method, fill=significant)) + geom_tile(color="white", size=1) + ggtitle("Dual Platelets") + xlab(label="marker") + facet_wrap(~class, scales = "free_x") + theme(text = element_text(size = 16),  axis.text.x = element_text(angle = 45, hjust=1))
