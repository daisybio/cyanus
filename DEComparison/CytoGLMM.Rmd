...
# Test diffcyt methods on covid platelet data
```{r}
# covid vs healthy in baseline platelets
...
```{r}
ei(sce_baseline)
markers_to_test <- getMarkersToTest(sce_baseline,"LMM","all")
LMM_res <- diffcyt::diffcyt(d_input = sce_baseline,
                                formula = createFormula(ei(sce_baseline), cols_fixed = c("covid") , cols_random = c("sample_id")),
