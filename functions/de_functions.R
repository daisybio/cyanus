    ...
    if (is.null(parameters[[method]]) &
      ...
      message(
        paste(
          ...
          ", random effects:",
          toString(random_effects),
          ", contrast:",
          toString(contrast_vars)
        )
      )
      parameters[[method]] <-
        prepDiffExp(
...
runDS <- function(sce,
  ...
  if (time_methods) {
    ...
    if ("type" %in% markers_to_test |
        "state" %in% markers_to_test) {
      sce <-
...
getMarkersToTest <- function(sce, ds_method, markers) {
  ...
  if (ds_method %in% c("diffcyt-DS-limma", "diffcyt-DS-LMM")) {
    ...
    is_marker <-
      SummarizedExperiment::rowData(sce)$marker_class %in% c("type", "state")
    if (by == "Marker by Class") {
      markers_to_test <-
        (SummarizedExperiment::rowData(sce)$marker_class %in% markers)[is_marker]
    } else{
    ...
    if (by == "Marker by Class") {
      markers_to_test <-
        SummarizedExperiment::rowData(sce)[SummarizedExperiment::rowData(sce)$marker_class %in% markers, ]$marker_name
    } else{
...
prepDiffExp <-
  function(sce,
    ...
    if (method %in% c("diffcyt-DA-edgeR", "diffcyt-DA-voom", "diffcyt-DS-limma")) {
      parameters[["design"]] <-
        diffcyt::createDesignMatrix(parameters[["ei"]], cols_design = colsDesign)
      parameters[["contrast"]] <-
        createCustomContrastMatrix(sce, contrastVars, parameters[["design"]], designMatrix = T)
      
    } else if (method == "diffcyt-DS-LMM") {
      parameters[["formula"]] <-
        diffcyt::createFormula(parameters[["ei"]],
                               cols_fixed = colsFixed,
                               cols_random = colsRandom)
      parameters[["contrast"]] <-
        createCustomContrastMatrix(
          sce,
          contrastVars,
          diffcyt::createDesignMatrix(parameters[["ei"]], cols_design = colsFixed),
          designMatrix = T
        )
    } else{
      parameters[["formula"]] <-
        diffcyt::createFormula(parameters[["ei"]],
                               ...
                               cols_random = colsRandom)
      parameters[["contrast"]] <-
        createCustomContrastMatrix(sce, contrastVars, colsFixed, designMatrix = F)
    }
    return(parameters)
  }
...
createCustomContrastMatrix <-
  function(sce, contrastVars, matrix, designMatrix = T) {
    if (designMatrix) {
...
doConditionSubselection <-
  function(sce,
           subselected_categories,
           mappedCategories,
           contrastVars,
           exclusionList,
           method) {
    catCount <-
      sapply(colnames(metadata(sce)$experiment_info)[!colnames(metadata(sce)$experiment_info) %in% c("n_cells", "sample_id")], function(x) {
        return(0)
      })
    names(catCount) <-
      colnames(metadata(sce)$experiment_info)[!colnames(metadata(sce)$experiment_info) %in% c("n_cells", "sample_id")]
    exclude <- list()
    ...
    for (x in names(catCount)) {
      if (x %in% contrastVars & catCount[[x]] == 1) {
        message(
          "You want to analyse a condition you subsetted. That is not meaningful. Try again.",
          type = "error"
        )
        return(NULL)
      }
...
runDS_old <- function(sce,
  ...
  if ("limma" %in% ds_methods) {
    ...
    parameters <-
      prepDiffExp(
        sce,
        contrastVars = c(condition),
        colsDesign = c(condition),
  ...
  if ("diffcyt-DS-LMM" %in% ds_methods) {
    ...
    parameters <-
      prepDiffExp(
        sce,
        contrastVars = c(condition),
        colsFixed = c(condition),
        colsRandom = c(random_effect),
        method = "diffcyt-DS-LMM"
      )
    
    markers_to_test <- getMarkersToTest(sce, "LMM", features)
    LMM_res <- diffcyt::diffcyt(
      d_input = sce,
      formula = parameters[["formula"]],
      contrast = parameters[["contrast"]],
      analysis_type = "DS",
  ...
  if ("SigEMD" %in% ds_methods) {
    ...
    nperm <-
      ifelse(is.null(extra_args$nperm), 100, extra_args$nperm)
    permute_samples <-
      ifelse(is.null(extra_args$permute_samples),
             FALSE,
             extra_args$permute_samples)
    ...
    SigEMD_res <-
      SigEMD(
        ...
        k,
        condition,
        Hur_gene = rownames(sce_SigEMD),
        nperm = nperm,
        parallel = parallel,
        permute_samples = permute_samples
      )
    SigEMD_res$overall <-
      data.table::rbindlist(lapply(SigEMD_res, function(x)
        x$emdall))
  ...
  if ("DEsingle" %in% ds_methods) {
    ...
    DEsingle_res <-
      DEsingleSCE(sce_DEsingle, condition, k, parallel = parallel)
